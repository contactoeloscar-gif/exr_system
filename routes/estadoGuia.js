...
function canUserOperateTransition(req, guia, toEstado) {
  ...
  // Operación origen / colecta
  if (toEstado === "RECIBIDO_ORIGEN" || toEstado === "EN_TRANSITO_A_CENTRAL") {
    return userSucursalId === origenId;
  }

  // Operación HUB / distribución
  if (
    toEstado === "RECIBIDO_CENTRAL" ||
    toEstado === "RECIBIDO_CENTRAL_OBSERVADO" ||
    toEstado === "EN_TRANSITO_A_DESTINO"
  ) {
    return userSucursalId === destinoId || userSucursalId === origenId;
  }

  // Operación destino / entrega
  if (
    toEstado === "RECIBIDO_DESTINO" ||
    toEstado === "RECIBIDO_DESTINO_OBSERVADO" ||
    toEstado === "ENTREGADO"
  ) {
...
router.post(
  "/",
  canOperateGuia({ allowOrigen: true, allowDestino: true }),
  async (req, res) => {
    ...
    try {
      ...
      const guia = q.rows[0];
      const fromEstado = normEstado(guia.estado_logistico);

      if (!canUserOperateTransition(req, guia, estado)) {
        await client.query("ROLLBACK");
        return res.status(403).json({
          ok: false,
          error: "Sin permisos para operar esta transición",
        });
      }
      ...
      // Pago en destino requiere cobro o excepción antes de entrega
      if (
        estado === "ENTREGADO" &&
